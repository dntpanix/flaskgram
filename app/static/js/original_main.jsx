...
// Використовуємо тот ож домен та префікс /api
const API_URL = '/api';
...
function InstagramClone() {
  ...
  useEffect(() => {
    // якщо токен в localStorage — підвантажити
  ...
  const fetchUserProfile = async () => {
    try {
      const r = await fetch(`${API_URL}/user/me`, { headers: { 'Authorization': `Bearer ${token}` } });
      if (r.ok) {
  ...
  const fetchPosts = async () => {
    try {
      const r = await fetch(`${API_URL}/posts`, { headers: { 'Authorization': `Bearer ${token}` } });
      if (r.ok) {
        const data = await r.json();
        setPosts(data.posts || []);
      }
  ...
  const logout = () => { setToken(null); setUser(null); setPosts([]); setView('login'); };

  const Icon = ({ icon: IconComponent, ...props }) => React.createElement(IconComponent, props);

  // --- UI (той самий JSX з вашого index.html) ---
  if (!token) {
    return (
            ...
            {view === 'login' ? (
              ...
              <div className="space-y-2">
                ...
                <input type="password" placeholder="Password" value={signupData.password}
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm bg-gray-50 focus:outline-none focus:border-gray-400"/>
                <input type="password" placeholder="Confirm Password" value={signupData.confirm_password}
  ...
  // rest of authenticated UI (home/create/profile) — такий же як в оригіналі
  return (
                ...
                {posts.map((post, idx) => (
                  <div key={idx} className="bg-white border border-gray-300 rounded-lg">
                    ...
                    <div className="w-full aspect-square bg-gray-100 flex items-center justify-center">
                      {post.image_url ? (<img src={post.image_url} alt={post.caption} className="w-full h-full object-cover" />) : (<Icon icon={Camera} size={48} className="text-gray-400" />)}
                    </div>
            ...
            <div className="bg-white border border-gray-300 rounded-lg p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="url" placeholder="https://example.com/image.jpg" value={newPost.image_url}
                ...
                <div>
                  ...
                  <textarea placeholder="Write a caption..." value={newPost.caption}
                ...
                <button onClick={createPost} disabled={loading} className="w-full bg-blue-500 text-white py-2 rounded font-semibold hover:bg-blue-600 disabled:bg-blue-300">
                ...
                <div className="grid grid-cols-3 gap-1">
                  {posts.filter(p => p.author?.username === user?.username).map((post, idx) => (
                    <div key={idx} className="aspect-square bg-gray-100 flex items-center justify-center">
                      {post.image_url ? (<img src={post.image_url} alt={post.caption} className="w-full h-full object-cover" />) : (<Icon icon={Camera} size={32} className="text-gray-400" />)}
                    </div>
                  ))}
                </div>
              </div>
